$(function(){
	...
	$('#form').w2form({
		...
		{ name: 'dataHoraInicio', type: 'date', required: false, html: { caption: 'Data Início', attr: 'size="40" maxlength="16" readonly' } },
		{ name: 'horaInicio', type: 'time', required: false, html: { caption: 'Hora Início', attr: 'size="40" maxlength="16" readonly' } },
		{ name: 'dataHoraFim', type: 'date', required: false, html: { caption: 'Data Fim', attr: 'size="40" maxlength="64" readonly' } },
		{ name: 'horaFim', type: 'time', required: false, html: { caption: 'Hora Fim', attr: 'size="40" maxlength="64" readonly' } },
		{ name: 'dataHoraPrevisaoInicio', type: 'date', required: false, html: { caption: 'Data Previsão Início', attr: 'size="40" maxlength="32" readonly' } },
		{ name: 'horaPrevisaoInicio', type: 'time', required: false, html: { caption: 'Hora Previsão Início', attr: 'size="40" maxlength="32" readonly' } },
		{ name: 'dataHoraPrevisaoFim', type: 'date', required: false, html: { caption: 'Data Previsão Fim', attr: 'size="40" maxlength="32" readonly' } },
		{ name: 'horaPrevisaoFim', type: 'time', required: false, html: { caption: 'Hora Previsão Fim', attr: 'size="40" maxlength="32" readonly' } },
		{ name: 'descricao', type: 'text', required: false, html: { caption: 'Descrição', attr: 'size="40" maxlength="16" readonly' } },
		{ name: 'funcionario', type: 'list', required: false, options: {items: funcionario}, html: { caption: 'Funcionário', attr: 'size="40" readonly' } },
		{ name: 'tipoAtividade', type: 'list', required: false, options: { items: tipoAtividade }, html: { caption: 'Tipo Atividade', attr: 'size="40" readonly' } }
		],
		actions: {
			Reset: function () {
				this.clear();
	...
	function requestInsert(record) {
		...
		$.ajax({
			...
			success: function(data) {
				w2popup.open({
					title: 'Sucesso',
					body: '<div class="w2ui-centered">Atividade fechada com sucesso.</div>',
					showMax: true
				});
				requestFindAll();
			},
			error: function(jqXHR, ajaxOptions, thrownError) {
				w2popup.open({
					title: 'Erro',
					body: '<div class="w2ui-centered">Erro ao fechar a atividade.<br/>' + thrownError + jqXHR.responseText + '</div>',
					showMax: true
				});
			}
		});
	...
	function requestUpdate(record) {
		...
		record.dataHoraInicio = dateSplit(dataHoraInicio.value, horaInicio.value);
		record.dataHoraFim = dateFormat(new Date()); 
		record.dataHoraPrevisaoInicio = dateSplit(dataHoraPrevisaoInicio.value,horaPrevisaoInicio.value);
		record.dataHoraPrevisaoFim = dateSplit(dataHoraPrevisaoFim.value,horaPrevisaoFim.value);
